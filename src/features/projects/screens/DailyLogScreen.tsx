        ...
        return (
            <TouchableOpacity style={styles.logCard} onPress={() => openEditModal(item)}>
                ...
                <View style={styles.performanceRow}>
                    {!item.horaInicio ? (
                        <TouchableOpacity
                            style={[styles.perfBtn, { backgroundColor: C.success + '20' }]}
                            onPress={() => updateLog(item.id, item.version || 1, { horaInicio: new Date().toISOString() })}
                        >
                            <Icon name="play" size={14} color={C.success} />
                            <Text style={[styles.perfBtnText, { color: C.success }]}>Iniciar Jornada</Text>
                        </TouchableOpacity>
                    ) : (
                    ...
                    {item.horaInicio && !item.horaFin && (
                        <TouchableOpacity
                            style={[styles.perfBtn, { backgroundColor: C.danger + '20', marginLeft: 8 }]}
                            onPress={() => updateLog(item.id, item.version || 1, { horaFin: new Date().toISOString() })}
                        >
                            <Icon name="square" size={14} color={C.danger} />
                            <Text style={[styles.perfBtnText, { color: C.danger }]}>Finalizar Jornada</Text>
                        </TouchableOpacity>
                    )}
                ...
                <View style={styles.footerInfo}>
                    <View style={styles.footerItem}>
                        <Icon name="users" size={14} color={C.textSecondary} />
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                ListFooterComponent={loadingMore ? <ActivityIndicator size="small" color={C.primary} style={{ margin: 20 }} /> : null}
                ListEmptyComponent={
                    <EmptyState 
                        icon="book" 
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <View style={styles.inputGroup}>
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                                ...
                                <TouchableOpacity style={styles.addPhotoBtn} onPress={pickImage}>
                                    <Icon name="image" size={22} color={C.textMuted} />
                                    <Text style={{ color: C.textMuted, fontSize: 9, marginTop: 2 }}>Galería</Text>
                                </TouchableOpacity>
                                <TouchableOpacity style={[styles.addPhotoBtn, { marginLeft: 6 }]} onPress={takePhoto}>
                                    <Icon name="camera" size={22} color={C.textMuted} />
                                    <Text style={{ color: C.textMuted, fontSize: 9, marginTop: 2 }}>Cámara</Text>
                                </TouchableOpacity>
                            </ScrollView>
                        </View>
                        ...
                        <View style={styles.modalActions}>
                            ...
                            <TouchableOpacity style={[styles.confirmBtn, { backgroundColor: isUploading ? C.textMuted : C.primary }]} onPress={handleCreateOrUpdateLog} disabled={isUploading}>
            ...
            <Modal visible={viewerVisible} transparent animationType="fade">
                <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.96)', justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    >
                        <Icon name="x" size={26} color={C.white} />
                    </TouchableOpacity>
                    <Image
...
function makeStyles(C: ThemeColors) {
    return StyleSheet.create({
        ...
        logCard: {
            backgroundColor: C.surface, padding: SPACING.md, borderRadius: RADIUS.md,
            marginBottom: SPACING.md, borderWidth: 1, borderColor: C.border, ...SHADOWS.sm,
        },
        logHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingBottom: SPACING.md, borderBottomWidth: 1, borderBottomColor: C.border, marginBottom: SPACING.md },
        dateLabel: { flexDirection: 'row', alignItems: 'center' },
        dateText: { color: C.white, fontWeight: 'bold', fontSize: FONTS.sizes.md, marginLeft: SPACING.sm, textTransform: 'capitalize' },
        weatherBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: C.surfaceLight, paddingHorizontal: 8, paddingVertical: 4, borderRadius: RADIUS.round },
        weatherText: { fontSize: 10, fontWeight: 'bold', marginLeft: 4 },
        activitiesTitle: { color: C.textSecondary, fontSize: FONTS.sizes.xs, textTransform: 'uppercase', marginBottom: 4 },
        activitiesText: { color: C.white, fontSize: FONTS.sizes.md, lineHeight: 22, marginBottom: SPACING.md },
        obsContainer: { flexDirection: 'row', backgroundColor: C.warning + '20', padding: SPACING.sm, borderRadius: RADIUS.sm, marginBottom: SPACING.md },
        obsText: { color: C.warning, fontSize: FONTS.sizes.sm, marginLeft: SPACING.sm, flex: 1 },
        thumbnailCard: { width: 72, height: 72, borderRadius: RADIUS.sm, marginRight: 8 },
        footerInfo: { flexDirection: 'row', justifyContent: 'flex-start', paddingTop: SPACING.sm, borderTopWidth: 1, borderTopColor: C.border },
        footerItem: { flexDirection: 'row', alignItems: 'center', marginRight: SPACING.lg },
        footerText: { color: C.textSecondary, fontSize: FONTS.sizes.sm, marginLeft: 6 },
        ...
        perfTimeLabel: { color: C.textMuted, fontSize: 11 },
        perfTimeValue: { color: C.white, fontSize: 12, fontWeight: 'bold' },
        durationBadge: { backgroundColor: C.primary + '30', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 4, marginLeft: 'auto' },
        durationText: { color: C.info, fontSize: 11, fontWeight: 'bold' },
        ...
        inputLabel: { color: C.textSecondary, marginBottom: 8 },
        input: { backgroundColor: C.surfaceLight, color: C.white, padding: SPACING.md, borderRadius: RADIUS.md },
        climaRow: { flexDirection: 'row', justifyContent: 'space-between' },
        climaBtn: { flex: 1, padding: SPACING.sm, borderRadius: RADIUS.md, backgroundColor: C.surfaceLight, alignItems: 'center', marginHorizontal: 4 },
        climaBtnActive: { backgroundColor: C.primary },
        climaBtnText: { color: C.textSecondary, fontSize: FONTS.sizes.sm },
        climaBtnTextActive: { color: C.white, fontWeight: 'bold' },
        addPhotoBtn: { width: 60, height: 60, borderRadius: RADIUS.sm, backgroundColor: C.surfaceLight, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: C.border, borderStyle: 'dashed', marginLeft: 8 },
        thumbnailContainer: { marginRight: 8, position: 'relative' },
        deleteImageBtn: { position: 'absolute', top: -5, right: -5, backgroundColor: C.danger, width: 20, height: 20, borderRadius: 10, justifyContent: 'center', alignItems: 'center', zIndex: 10 },
